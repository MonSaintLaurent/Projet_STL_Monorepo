...
export default function CreatePouleSection({ onBack }: CreatePouleSectionProps) {
    ...
    // Initialise players avec les amis existants
    useEffect(() => {
    if (!loadingFriends && friends.length > 0) {
        setPlayers(prev => {
        // Pas de doublons
        const existingIds = new Set(prev.map(p => p.id));
        const newPlayers = friends
            .filter(f => !existingIds.has(f.id))
            .map(f => ({ ...f, selected: false })); // Non sélectionnés par défaut
            ...
            <div className="create-form">
                ...
                <div className="form-group">
                    ...
                    <div className="players-grid" style={{ marginTop: "0.5rem" }}>
                        {players.length === 0 && <div style={{ color: "#6b7280" }}>Aucun joueur à afficher</div>}
